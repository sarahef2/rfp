        ...
        verbose = FALSE)
{
  ...
  if (is.null(variable.weight)) variable.weight = rep(1/p, p) else {variable.weight = variable.weight/max(variable.weight)} #Changed to max from sum to fit the new variable weighting methods
  ...
  if (length(variable.weight) != p) {warning("Variable weights length must by p, reset to equal weights"); variable.weight = rep(1/p, p); use.var.weight = FALSE}
